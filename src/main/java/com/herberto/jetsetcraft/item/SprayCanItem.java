...
public final class SprayCanItem extends Item {
    ...
    public SprayCanItem(Properties properties) { super(properties); }
    @Override public InteractionResult useOn(UseOnContext context) {
        Direction face = context.getClickedFace();
        if (!face.getAxis().isHorizontal() || !JetSetConfig.SERVER.allowGraffiti.get()) return InteractionResult.FAIL;
        ...
        if (player != null && player.isShiftKeyDown()) {
            if (!context.getLevel().isClientSide) {
                ...
                player.displayClientMessage(Component.translatable("message.jetsetcraft.graffiti_variant", GraffitiCatalog.id(next)).withStyle(ChatFormatting.LIGHT_PURPLE), true);
            }
        ...
        if (context.getLevel() instanceof ServerLevel serverLevel) {
            ...
            AABB patch = new AABB(graffiti.position(), graffiti.position()).inflate(
                    Math.max(0.14, entry.renderWidth() * 0.48), Math.max(0.55, entry.renderHeight() * 0.48), Math.max(0.14, entry.renderWidth() * 0.48));
            serverLevel.getEntitiesOfClass(GraffitiEntity.class, patch, e -> e.getFace() == face).forEach(GraffitiEntity::discard);
            serverLevel.addFreshEntity(graffiti);
            if (player != null && !player.getAbilities().instabuild) stack.hurtAndBreak(1, player, p -> p.broadcastBreakEvent(context.getHand()));
        }
        return InteractionResult.sidedSuccess(context.getLevel().isClientSide);
    }
}
